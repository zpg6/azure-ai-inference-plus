...
from azure_ai_inference_plus import ChatCompletionsClient, UserMessage, AzureKeyCredential
...
class TestReasoningFunctionality:
    ...
    def test_reasoning_with_json_mode(self):
        ...
        mock_choice = Mock()
        mock_message = Mock()
        mock_message.content = '<think>Let me format this as JSON</think>{"result": "success"}'
    ...
    def test_reasoning_with_non_json_mode(self):
        ...
        with patch.object(client.__class__.__bases__[0], 'complete') as mock_complete:
            ...
            assert result.choices[0].message.content == '<think>Let me think about this</think>The answer is 42.'
    ...
    def test_no_reasoning_tags(self):
        ...
        with patch.object(client.__class__.__bases__[0], 'complete') as mock_complete:
            ...
            with patch('azure_ai_inference_plus.client.process_response_with_reasoning') as mock_process:
                result = client.complete(
                    messages=[UserMessage("test")],
                    model="gpt-4"
                )
                ...
                assert result.choices[0].message.content == '<think>This looks like reasoning but no tags configured</think>Regular response.'
